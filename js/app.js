...
Muchas gracias por tu compra y por apoyar nuestro emprendimiento. 💛`;
    ...
    function mobilePaymentCheckControl(order){
      const stateValue = paymentChecklistState(order);
      if(stateValue === 'paid'){
        return `<button class="mobile-order-check is-paid" type="button" disabled aria-label="Pago confirmado" title="Pago confirmado y bloqueado"><span>✓</span></button>`;
      }
      if(stateValue === 'partial'){
        return `<button class="mobile-order-check is-partial" type="button" onclick="markOrderPaidFromChecklist('${order.id}')" aria-label="Pago parcial. Falta ${money(saldo(order))}" title="Tiene un abono parcial. Completa el saldo antes de marcarlo como pagado"><span></span></button>`;
      }
      return `<button class="mobile-order-check" type="button" onclick="markOrderPaidFromChecklist('${order.id}')" aria-label="Marcar pedido como pagado" title="Marcar como pagado"><span></span></button>`;
    }

    function mobileReminderButton(order){
      const disabled = saldo(order) <= 0 ? 'disabled title="Este pedido ya aparece como pagado"' : '';
      return `<button class="mobile-order-action whatsapp" type="button" ${disabled} onclick="sendPaymentReminder('${order.id}')">Recordatorio</button>`;
    }
    ...
    function renderOrders(){
      ...
      const desktopRows = orders.map(o=>{
        const b = batchById(o.batchId);
      ...
      const mobileRows = orders.map(o=>{
        const balance = saldo(o);
        const stateValue = paymentChecklistState(o);
        const paymentText = stateValue==='paid' ? 'Pagado' : stateValue==='partial' ? `Debe ${money(balance)} · abonó ${money(o.paid)}` : `Debe ${money(balance)}`;
        return `<div class="mobile-order-row ${stateValue==='paid'?'is-paid':stateValue==='partial'?'is-partial':''}">
          <div class="mobile-order-client">
            <button class="mobile-order-client-name" type="button" onclick="openOrderDetail('${o.id}')">${escapeHTML(o.client)}</button>
            <small>${escapeHTML(paymentText)}</small>
          </div>
          <div class="mobile-order-check-cell">${mobilePaymentCheckControl(o)}</div>
          <div class="mobile-order-actions">
            ${balance>0?`<button class="mobile-order-action ok" type="button" onclick="openPaymentModal('${o.id}')">Abono</button>`:'<button class="mobile-order-action paid-action" type="button" disabled>Pagado</button>'}
            ${mobileReminderButton(o)}
          </div>
        </div>`;
      }).join('');

      document.getElementById('orderTable').innerHTML = `
        <div class="orders-desktop-view table-wrap action-table"><table><thead><tr><th>Lote</th><th>Cliente</th><th>Producto</th><th class="number">Cant.</th><th class="number">Total</th><th class="number">Pagó</th><th class="number">Saldo</th><th>Pagado</th><th>Entrega</th><th>Acciones</th></tr></thead><tbody>${desktopRows}</tbody></table></div>
        <div class="orders-mobile-view" aria-label="Lista de pedidos">
          <div class="mobile-order-list-head"><span>Cliente</span><span>Pagado</span><span>Acciones</span></div>
          <div class="mobile-order-list">${mobileRows}</div>
        </div>`;
    }
